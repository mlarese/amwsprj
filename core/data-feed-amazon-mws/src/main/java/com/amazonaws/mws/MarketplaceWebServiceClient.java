...
public  class MarketplaceWebServiceClient implements MarketplaceWebService {
    ...
    private <T, U> T invoke(Class<T> clazz, Map<String, String> parameters, U request ) throws MarketplaceWebServiceException {
        ...
        try {
            ...
            int retries = 0;
            do {
                log.info("Sending Request to host:  " + config.getServiceURL());
                ...
                try {
                    ...
                    if (status == HttpStatus.SC_OK && responseBodyString != null) {
                        ...
                        if (status == HttpStatus.SC_INTERNAL_SERVER_ERROR
                            ...
                            log.info(status == HttpStatus.SC_SERVICE_UNAVAILABLE);
                            log.info(!(error.getCode().equals("RequestThrottled")));
                            log.info(!(request instanceof SubmitFeedRequest));
                            log.info(pauseIfRetryNeeded(++retries));
    ...
    private ResponseHeaderMetadata getResponseHeaderMetadata(HttpResponse response) {
      ...
      return new ResponseHeaderMetadata(
        ...
        quotaMax != null ? quotaMax : null,
        quotaRemaining != null ? quotaRemaining : null,
        quotaResetDate != null ? quotaResetDate : null)
        ;
    }
